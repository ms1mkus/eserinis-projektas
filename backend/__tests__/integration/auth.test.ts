...
import { resetDatabase, teardownDatabase, createAuthenticatedAgent, setupServer } from '../utils/testsHelpers';
...
describe('Auth routes', () => {
    ...
    test('Log user by username', async () => {
        ...
        const res = await request(server).post('/api/auth/login').send({ login: name, password });
    ...
    test('Log user by email', async () => {
        const emailAddress = 'fakeUser@gmail.com';
        const password = 'fakeUserPwd';
        const user = await registerTestUser({ emailAddress, password });

        const res = await request(server).post('/api/auth/login').send({ login: emailAddress, password });
    ...
    test('Login fails if wrong credentials', async () => {
        ...
        // Wrong password
        const res2 = await request(server).post('/api/auth/login').send({ login: name, password: 'wrongPassword' });
    ...
    test('Throw an error if authenticated user tries to login', async () => {
        const name = 'fakeUser';
        const password = 'fakeUserPwd';
        const { agent } = await createAuthenticatedAgent(server, { name, password });

        const res = await agent.post('/api/auth/login').send({ login: name, password });
    ...
    test('Throw an error if an error occurs during Express login', async () => {
        ...
        const res = await request(serverFailingLogIn).post('/api/auth/login').send({ login: name, password });
