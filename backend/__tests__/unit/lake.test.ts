...
describe("getLakeInfo", () => {
    ...
    // Šitą naudojame kaip konstantą, kad gražiai pamock'inti Response objektą, nuoroda (Geras guide) https://codewithhugo.com/express-request-response-mocking/
    const mockResponse = () => {
        const res: any = {};
    ...
    it("should fetch and return lake info", async () => {
        const mockFishes = [{ name: "Fish1", username: "User1", count: 1, id: 1 }];
        const res = mockResponse();
        (mockQueryRunner.manager.query as jest.Mock).mockResolvedValue(mockFishes);
        await lakeController.getLakeInfo(mockRequest as Request, res as Response);
        expect(mockQueryRunner.manager.query).toHaveBeenCalledWith(expect.any(String), [mockRequest.params.lakeId]);
    ...
    it("should handle errors", async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({ message: "Failed to fetch lake info", error: mockError.message });
    });
});
...
describe("likeLake", () => {
    ...
    it("should like a lake", async () => {
        ...
        (mockQueryRunner.manager.getRepository as jest.Mock)
            .mockReturnValueOnce({ findOneOrFail: jest.fn().mockResolvedValue(mockUser) }) // userRepository
            .mockReturnValueOnce({ findOneOrFail: jest.fn().mockResolvedValue(mockLake) }) // lakeRepository
            .mockReturnValueOnce({ save: jest.fn().mockResolvedValue(mockLike) }); // likeRepository

        await lakeController.likeLake(mockRequest as Request, res as Response);
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith({ success: true, message: "Lake liked" });
    });
    ...
    it("should unlike a lake", async () => {
        ...
        await lakeController.likeLake(mockRequest as Request, res as Response);
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith({ success: true, message: "Lake unliked" });
    });
...
describe("getLakes", () => {
    ...
    it("should return lakes with a matching `isLiked` tag based on the user's likes", async () => {
        ...
        (mockQueryRunner.manager.getRepository as jest.Mock).mockReturnValue({ find: jest.fn().mockResolvedValue(mockLakes) });
